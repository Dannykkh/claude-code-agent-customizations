...
| 외부 스킬 통합 (TDD, 디버깅, Semgrep 등) | ✅ 완성 |
| MCP 설정 자동 설치 (install-mcp.js) | ✅ 완성 |
...
| excel, xlsx, markdown, 엑셀변환 | [#tools/excel2md](#excel2md) |
...
| gepetto, 심층인터뷰, 디자인비전, 쉬운질문 | [#tools/gepetto-interview-v2](#gepettointerviewv2) |
...
- `interview-protocol.md`에 5개 구조화 카테고리 추가 (A~E) ❌ SUPERSEDED → `#gepetto-interview-v2` (A~G, 7개 카테고리)
...
외부 레포에서 6개 스킬을 복사하여 통합:

| 스킬 | 출처 | 용도 |
|------|------|------|
| test-driven-development | obra/superpowers | TDD 워크플로우 |
| systematic-debugging | obra/superpowers | 체계적 디버깅 (11파일) |
| semgrep-rule-creator | trailofbits/skills | 보안 정적 분석 규칙 생성 |
| wrangler | cloudflare/skills | Cloudflare Workers CLI |
| docx | anthropics/skills | Word 문서 생성/편집 |
| pdf | anthropics/skills | PDF 처리/변환 |
...
MCP 서버 설정 자동 설치 시스템:
- `mcp-configs/*.json`: 5개 사전 설정 (context7, playwright, fetch, sequential-thinking, github)
- `install-mcp.js`: `--list`, `--all`, `--uninstall`, `--target` 지원
- 기본 대상: `~/.claude/settings.json` (글로벌)
- API 키 필요 MCP는 경고 표시, 기존 설정 보존
- install.bat 6단계 → 7단계 ([7/7] MCP 안내)
- **참조**: [2026-02-07 대화](.claude/conversations/2026-02-07.md)

### gepetto-interview-v2 ✅ CURRENT
`tags: gepetto, 심층인터뷰, 디자인비전, 쉬운질문, 5-whys, 벤치마킹`
`date: 2026-02-07`
`supersedes: #gepetto-verify의 인터뷰 카테고리 부분`

gepetto 인터뷰 프로토콜 대폭 개선:
...
- Claude Code Agent Teams 실험적 기능 활성화
- `~/.claude/settings.json`에 `"env": {"CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"}` 추가
- 팀 리드가 여러 Claude 인스턴스를 병렬 조율 (네이티브 지원)
- 기존 orchestrator PM-Worker와 유사하지만 Claude Code 내장
- **참조**: [2026-02-07 대화](.claude/conversations/2026-02-07.md)
